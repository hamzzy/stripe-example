...
def safe_target(root: Path, relative: str) -> Path:
    """Resolve one admitted repository path without crossing the checkout."""
    if not relative or "\x00" in relative or Path(relative).is_absolute():
        raise ExecutorError("Planned path is invalid")
    try:
        target = (root / relative).resolve(strict=True)
        target.relative_to(root)
    except (OSError, ValueError) as exc:
        raise ExecutorError(f"Planned path is unavailable: {relative}") from exc
...
def _parse_check(root: Path, changed_paths: list[str]) -> dict:
    ...
    for relative in changed_paths:
        if not relative.endswith(".py"):
            continue
        try:
            ast.parse(safe_target(root, relative).read_text(encoding="utf-8"))
        except SyntaxError:
            return {
                "check_type": "parse",
                "status": "failed",
...
def _secret_check(root: Path, changed_paths: list[str]) -> dict:
    ...
    for relative in changed_paths:
        try:
            content = safe_target(root, relative).read_text(encoding="utf-8")
        except (ExecutorError, OSError, UnicodeDecodeError):
            continue
...
def main() -> int:
    ...
    dispatch_id = os.environ["NOMOS_DISPATCH_ID"]
    expected_input_digest = os.environ["NOMOS_ACTION_INPUT_DIGEST"]
    ...
    received_input_digest = hashlib.sha256(
        json.dumps(spec, sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
    if not re.fullmatch(r"[0-9a-f]{64}", expected_input_digest):
        raise ExecutorError("Dispatch contains an invalid action input digest")
    if received_input_digest != expected_input_digest:
        raise ExecutorError("Maintenance job inputs do not match the admitted dispatch")
    execution_mode = str(spec.get("execution_mode") or "")
    if execution_mode not in {"patch", "baseline"}:
        raise ExecutorError("Maintenance job has an invalid execution mode")
    ...
    try:
        # The workflow event runs at the default branch HEAD, which may have
        # moved since assessment. Stop before editing unless the checkout can
        # be pinned to the exact admitted base SHA.
        base_commit = str(spec["base_commit"])
        dirty = subprocess.check_output(
            ["git", "status", "--porcelain"], cwd=root, text=True
        ).strip()
        if dirty:
            raise ExecutorError("Runner checkout is not clean before execution")
        subprocess.run(
        ...
        if checked_out != base_commit:
            raise ExecutorError("Runner checkout does not match the maintenance job base SHA")

        planned_paths = sorted({str(step["path"]) for step in spec.get("steps", [])})
        for path in planned_paths:
            safe_target(root, path)

        if execution_mode == "patch":
            ...
            for path, (additions, deletions) in sorted(totals.items()):
                digest = hashlib.sha256(safe_target(root, path).read_bytes()).hexdigest()
        ...
        if changed_files:
            patch = subprocess.check_output(
                ["git", "diff", "--binary", "--", *[item["path"] for item in changed_files]],
                cwd=root,
            )
        ...
        checks = build_checks(spec, root, check_paths)
        if (
            execution_mode == "patch"
            and changed_files
            and os.environ.get("NOMOS_PUSH_BRANCH", "false").lower() == "true"
            ...
            subprocess.run(
                ["git", "add", "--", *[item["path"] for item in changed_files]],
                cwd=root,
                check=True,
            )
