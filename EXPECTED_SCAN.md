...
| `stripe.node.request-option-api-key` | `api_key:` ×1 | `src/payments/legacy-request-options.ts` |
| `stripe.node.request-option-idempotency-key` | `idempotency_key:` ×1 | `src/payments/legacy-request-options.ts` |
| `stripe.node.request-option-stripe-account` | `stripe_account:` ×1 | `src/payments/legacy-request-options.ts` |
| `stripe.node.request-option-stripe-version` | `stripe_version:` ×1 | `src/payments/legacy-request-options.ts` |
| `stripe.node.request-option-stripe-version-camel` | `stripeVersion:` ×1 | `src/payments/legacy-request-options.ts` |

The demo now carries nine signed, primary-source-backed deterministic
transformations across four vendors:

| Vendor | Additional rule | Target |
| --- | --- | --- |
| OpenAI | `openai.node.chat-completions-to-responses-simple` | simple Chat Completions request to Responses |
| Slack | `slack.node.apps-connections-open-empty-args` | add the v7-required empty argument object |
| Twilio | `twilio.http.deprecated-region-host-to-us1` | `api.ie1.twilio.com` to `api.twilio.com` |

A fresh scan with bundled content seeded should therefore create nine distinct
maintenance jobs: six Stripe jobs and one each for OpenAI, Slack, and Twilio.
The two `subscriptions.del` call sites are two steps in one job, not two jobs.

`stripe.Subscription.delete` in the Python reconciler remains valid evidence
for scanning but is intentionally not a maintenance target. Stripe's Python
changelog does not support the previously proposed delete-to-cancel rename.

The fixture is pinned to **Stripe 9.16.0** (TypeScript) and **9.12.0** (Python)
on purpose. The old Node method and request-option aliases still exist in
Stripe 9 and are removed by Node 11/13. A fixture already on Stripe 18 has
nothing left to migrate. If you bump these pins, the migration targets stop
compiling and the codemod stages become untestable.
